...
// function load() { try { return JSON.parse(fs.readFileSync(REMINDERS_FILE, "utf-8")); } catch { return []; } }
// function save(reminders) { fs.writeFileSync(REMINDERS_FILE, JSON.stringify(reminders, null, 2), "utf-8"); }
...
async function execute(name, input) {
  switch (name) {
    ...
    case "reminder_list": {
      const active = db.reminders.getActive();
    ...
    case "reminder_delete": {
      const existing = db.reminders.getById(input.id);
      if (!existing) return `Erinnerung ${input.id} nicht gefunden.`;
      db.reminders.remove(input.id);
      return `Gelöscht: "${existing.text}"`;
    }
...
function advanceReminder(id) {
  db.reminders.advance(id);
}

function incrementFailCount(id) {
  return db.reminders.incrementFailCount(id);
}
