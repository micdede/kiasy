...
const terminal = {
  ...
  getCwd() {
    try {
      ...
      return row ? row.value : "/home/mcde/whatsapp-claude";
    } catch { return "/home/mcde/whatsapp-claude"; }
  },
...
const toolSettings = {
  isEnabled(filename) {
    ...
    if (!row) return true; // Nicht registriert = enabled by default
    return !!row.enabled;
  },
...
function migrate() {
  ...
  // 5. Roadmap: Initiale Items wenn leer
  try {
    const rmCount = rm.count.get().n;
    if (rmCount === 0) {
      rm.insert.run("Agentic Loops", "Mehrstufige Tasks ohne Nutzerinteraktion — verkettete Workflows, die der Agent selbstständig abarbeitet", "planned", "high", "Agent");
      rm.insert.run("PWA extern + Push Notifications", "Chat-PWA auf externem Host mit echtem SSL-Cert und iOS Web Push (VAPID)", "idea", "normal", "Frontend");
      rm.insert.run("Unified SQLite DB", "Memory, Reminders, Knowledge, Events in jarvis.db konsolidiert", "done", "high", "Backend");
      rm.insert.run("Chat-PWA im Monitor", "Eigene Chat-UI als installierbare Web-App mit Voice + TTS", "done", "high", "Frontend");
      rm.insert.run("WhatsApp-Abhängigkeit entfernt", "whatsapp-web.js + Puppeteer/Chromium deinstalliert, ~1 GB freigeräumt", "done", "normal", "Cleanup");
      console.log("[DB] 5 initiale Roadmap-Items erstellt");
    }
  } catch (e) {
    console.warn("[DB] Roadmap-Init Warnung:", e.message);
  }
}
...
module.exports = { db, messages, memory, reminders, notes, events, terminal, roadmap, workflows, toolSettings, close };
