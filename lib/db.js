...
// --- News Sources ---
const ns = {
  all: db.prepare("SELECT * FROM news_sources ORDER BY name"),
  enabled: db.prepare("SELECT * FROM news_sources WHERE enabled = 1 ORDER BY name"),
  byType: db.prepare("SELECT * FROM news_sources WHERE type = ? AND enabled = 1 ORDER BY name"),
  getById: db.prepare("SELECT * FROM news_sources WHERE id = ?"),
  insert: db.prepare("INSERT INTO news_sources (type, name, url, api_key, category, enabled, config) VALUES (?, ?, ?, ?, ?, ?, ?)"),
  update: db.prepare("UPDATE news_sources SET type=?, name=?, url=?, api_key=?, category=?, enabled=?, config=?, updated=datetime('now','localtime') WHERE id=?"),
  remove: db.prepare("DELETE FROM news_sources WHERE id = ?"),
  count: db.prepare("SELECT COUNT(*) as n FROM news_sources"),
};

const newsSources = {
  getAll() { return ns.all.all().map(nsRow); },
  getEnabled() { return ns.enabled.all().map(nsRow); },
  getByType(type) { return ns.byType.all(type).map(nsRow); },
  getById(id) { const row = ns.getById.get(id); return row ? nsRow(row) : null; },

  create(data) {
    const info = ns.insert.run(
      data.type, data.name, data.url, data.api_key || '',
      data.category || 'general', data.enabled !== undefined ? (data.enabled ? 1 : 0) : 1,
  ...
  update(id, data) {
    const existing = ns.getById.get(id);
    if (!existing) return null;
    ns.update.run(
      data.type ?? existing.type, data.name ?? existing.name,
      data.url ?? existing.url, data.api_key ?? existing.api_key,
  ...
  remove(id) { return ns.remove.run(id).changes > 0; },
  count() { return ns.count.get().n; },
};
...
module.exports = { db, messages, memory, reminders, notes, events, terminal, roadmap, delegations, workflows, toolSettings, newsSources, close };
